# ********************************************************************
# Name    : Config
# Summary : Provides functionality interacting with configuration
#           properties, many of which are included in  properties.conf,
#           also includes some functionality to manage passwords and
#           cloud deployments.
# ********************************************************************
...
GLOBAL_CONFIG_DICT = {}
PROD_CONFIG_FILE = os.path.join(ENMUTILS_PATH, 'etc', 'properties.conf')
TORUTILS_BASHRC_ENV_VARIABLES = ["LMS_HOST", "EMP", "ENM_URL", "DEPLOYMENTNAME"]
...
def get_encoded_password_and_decode(username, password):
    """
    Decodes the previously encoded password.

    :rtype:     str
    :return:    password
    """
    if username == 'workload_admin':
...
def load_config(tool_class='prod', force_reload=False):
    ...
    if not cache.has_key(cache_key) or force_reload:
        load_bashrc_env_variables()
        # Always load the production props
        load_config_from_file(PROD_CONFIG_FILE)

        # Load the internal props if the tool_class is set to int
        if "int" in tool_class.lower():
            internal_config_file_location = os.path.join(pkgutil.get_loader('enmutils_int').filename, 'etc',
                                                         'properties.conf')
            load_config_from_file(internal_config_file_location)
...
def load_bashrc_env_variables():
    """
    Loads variables from the .bashrc file for root user
    """
    bashrc_file = "/root/.bashrc"
    if os.path.isfile(bashrc_file):
        for key in TORUTILS_BASHRC_ENV_VARIABLES:
            cmd = "source {0}; echo ${1}".format(bashrc_file, key)
            rc, output = commands.getstatusoutput(cmd)
            if not rc and output:
                os.environ[key] = output.strip()
...
def update_config_file(config_file, new_property_name, new_property_value):
    ...
    with open(config_file) as source_config, open(temp_config_filename, 'w') as temp_config:
        for line in source_config.readlines():
...
def load_local_config():
    ...
    global GLOBAL_CONFIG_DICT
    try:
        from enmutils import local_properties
    except ImportError:
        return
...
def get_prop(key):
    ...
    :raises RuntimeError: if there is no configuration property in configuration dictionary
    """
...
def set_prop(key, value):
    ...
    :type: value: string
    :raise RuntimeError: if the value wasn't provided for the key
    """
...
def get_log_dir():
    ...
    if "ENMUTILS_LOG_DIR" in os.environ:
        return os.environ["ENMUTILS_LOG_DIR"]
...
def load_credentials_from_props(username_key="username", password_key="password"):
    ...
    internal_package = pkgutil.get_loader('enmutils_int')
