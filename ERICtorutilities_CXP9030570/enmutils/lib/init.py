# ********************************************************************
# Name    : INIT
# Summary : Used mainly by tools, to initialize logging, redis
#           connections, handle signals and timeouts.
# ********************************************************************
...
def global_init(run_type, tool_class, run_name, args=None, simplified_logging=False, execution_timeout=900):
    ...
    try:
        ...
        print log.red_text("Could not load configuration properties. The system is in an unexpected state or is not "
    ...
    try:
        ...
        print log.red_text("Could not initialize logging. The system is in an unexpected state or is not configured correctly: {}".format(e))  # NOSONAR
        exit(5)
    ...
    if run_name in CPU_TOOLS_MAP and multiprocessing.cpu_count() < CPU_TOOLS_MAP[run_name]:
        log.logger.warn('The number of CPU cores on this MS is less than the minimum required cores for this tool. '
                        'This tool may under-perform or hang as a result')
    ...
    try:
        ...
        log.logger.error("Could not verify that the general-purpose scratch directory exists. The system is in an "
                         "unexpected state or is not configured correctly.")
        exit(5)
...
def _set_timeout(timeout_value, run_name):
    ...
    :type timeout_value: str
    :param timeout_value: application timeout value to be set
    :type run_name: The name of the action being run
    :param run_name: str
    """
