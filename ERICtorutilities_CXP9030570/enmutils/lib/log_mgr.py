# ********************************************************************
# Name    : Log Manager
# Summary : Manages logging from tools, profiles and multiprocessing
#           of logs from profile daemons. Includes formatting, log
#           rotation and queue management.
# ********************************************************************
...
class UtilitiesLogManager(object):
    ...
    def init(self):
        ...
        log_handler = logging.handlers.WatchedFileHandler(os.path.join(config.get_log_dir(), config.get_prop("console_log_file")))
        log_handler.setLevel(logging.INFO)
        log_handler.setFormatter(log_formatter)
        self.logger.addHandler(log_handler)

        # Set up our debug log handler
        debug_handler = logging.handlers.WatchedFileHandler(os.path.join(config.get_log_dir(), config.get_prop("debug_log_file")))
        ...
        exception_handler = logging.handlers.WatchedFileHandler(os.path.join(config.get_log_dir(), config.get_prop("exception_log_file")))
...
class MultiProcessingLog(logging.Handler):
    ...
    def __init__(self, filepath):
        """
        Initialize logging to profiles.log

        """
        logging.Handler.__init__(self)

        self._handler = logging.handlers.WatchedFileHandler(filepath)
    ...
    def emit(self, record):
        """
        Send a record to the queue for processing.

        :param record: Record to be logged
        :type record: logging.LogRecord
        :raises KeyboardInterrupt: If logger stopped
        :raises SystemExit: If logger stopped
        """
