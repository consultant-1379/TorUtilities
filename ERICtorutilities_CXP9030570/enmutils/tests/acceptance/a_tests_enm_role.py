...
from enmutils.lib.enm_user_2 import EnmComRole, CustomRole, EnmRoleAlias, RoleCapability
from testslib import func_test_utils, test_utils, test_fixture
...
class EnmRoleAcceptanceTests(unittest2.TestCase):
    ...
    def setUp(self):
        ...
        self.alias_role = EnmRoleAlias(self.NAME, {EnmComRole("SystemAdministrator")}, description=self.DESCRIPTION, user=self.fixture.users[0])
        self.custom_role = CustomRole(self.NAME, description=self.DESCRIPTION, roles={EnmComRole("SystemAdministrator")},
                                      capabilities=RoleCapability.get_role_capabilities_for_resource("cm_editor"), user=self.fixture.users[0])
