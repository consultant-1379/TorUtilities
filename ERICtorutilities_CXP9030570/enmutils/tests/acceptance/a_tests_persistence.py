...
import enmutils
from enmutils.lib import persistence, filesystem
from testslib import func_test_utils, test_utils, test_fixture
from testslib.func_test_utils import func_dec

ENMUTILS_PATH = os.path.dirname(enmutils.__file__)
_STORAGE_PATH = os.path.join(ENMUTILS_PATH, '.persistence', 'persistence.db')
...
class PersistenceAcceptanceTests(unittest2.TestCase):
    ...
    def test_clear_function_with_no_persistence_file_runs_without_error(self):
        try:
            if filesystem.does_file_exist(_STORAGE_PATH):
                filesystem.delete_file(_STORAGE_PATH)
            persistence.clear()
        except RuntimeError:
