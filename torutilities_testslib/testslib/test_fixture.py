...
from enmutils.lib import log, config, exception, init, mutexer, persistence, timestamp, arguments
from enmutils.lib.enm_node_management import (FmManagement, CmManagement, PmManagement)
...
class TestFixture(object):
    def __init__(self, cls):
        ...
        # Nodes
        self.num_nodes = sum(val for val in cls.NUM_NODES.values() if val > 0 or val == -1) if hasattr(cls, 'NUM_NODES') else 0
        self.nodes = []
    ...
    def setup(self):
        ...
        self.admin_user = enm_user.get_workload_admin_user()

        # Remove this step when creating node without ossModelIdentity has been verified
    ...
    def _setup_users(self):
        ...
        for i in xrange(self.num_users):
            ...
            try:
                user.create(create_as=enm_user.get_workload_admin_user())
    ...
    def _teardown_users(self):
        ...
        :raises: RunTimeError: raises an exception if users fail to be deleted
        """
    ...
    def verify_nodes_exist(user, nodes, get_all=False):
        ...
        :param get_all: whether to get all network elements
        :type get_all: Boolean
        :rtype: set
        :returns: nodes that do not exist in ENM
        """
        found_nodes = []
    ...
    def verify_nodes_are_synced(user, nodes):
        ...
        @param nodes: The node need of the test case that calls test_fixture
        :rtype: list
        :returns: list of nodes not synced in either CM, FM, PM
        """
        found_nodes = []
    ...
    @property
    def add_error_as_exception(self, e):
        log.logger.error("Profile Exception thrown {0}".format(e))
...
class DvTestFixture(TestFixture):
    # Placeholder in case dv was to come back
    # Library to use for UI: https://splinter.readthedocs.io/en/latest/
    def __init__(self, *args, **kwargs):
        self.test_type = "dv"
        super(DvTestFixture, self).__init__(*args, **kwargs)
